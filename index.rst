...
.. py:class:: ZstdCompressor
    ...
    .. py:method:: compress(self, data, mode=ZstdCompressor.CONTINUE)
        ...
        .. hint:: Why there is a *mode* parameter?
            ...
            #. When reuse :py:class:`ZstdCompressor` object for big number of same type individual data, make operate conveniently.
            #. Can use :ref:`underlying zstd library's multi-threading compression<mt_compression>`.
            #. Can use :ref:`rich memory mode<rich_mem>`.
    ...
    .. py:method:: flush(self, mode=ZstdCompressor.FLUSH_FRAME)
        ...
        ``c.flush(c.FLUSH_FRAME)`` is equivalent to ``c.compress(b'', c.FLUSH_FRAME)``
    ...
    .. sourcecode:: python
        ...
        c1 = ZstdCompressor()
        dat1 = c1.compress(b'123456')
        dat2 = c1.compress(b'abcdef')
        dat3 = c1.flush()

        # reuse for same type individual data
        option = {CParameter.nbWorkers : 4}
        c2 = ZstdCompressor(option)
        compressed_dat1 = c2.compress(raw_dat1, c2.FLUSH_FRAME)
        compressed_dat2 = c2.compress(raw_dat2, c2.FLUSH_FRAME)
...
.. note:: Rich memory mode
    ...
    pyzstd module has a "rich memory mode" for compression. There is a *rich_mem* argument in function :py:func:`compress`, method :py:meth:`ZstdCompressor.__init__`.

    When use this mode for :py:class:`ZstdCompressor`, it only works in a single :py:attr:`ZstdCompressor.FLUSH_FRAME` compression, otherwise this mode will be ignored.

    Effects:

    * Allocate more memory, the output buffer is larger than input data a little.
    * When :ref:`zstd multi-threading compression<mt_compression>` is disabled, 10% ~ 15% faster.
    * When :ref:`zstd multi-threading compression<mt_compression>` is enabled, maybe the size of output data can be reduced a bit.
    ...
    .. sourcecode:: python
        ...
        # used with ZstdCompressor object
        c = ZstdCompressor(rich_mem=True)

        # only works in FLUSH_FRAME mode
        compressed_dat1 = c.compress(raw_dat1, c.FLUSH_FRAME)
        compressed_dat2 = c.compress(raw_dat2, c.FLUSH_FRAME)

    Compress a 520 MB data, best of 5 results:

    +-------------+------------------+----------------+
    | zstd threads| rich_mem=True    | rich_mem=False |
    +=============+==================+================+
    |           1 | **4.57 sec** [1] |   5.38 sec [2] |
    +-------------+------------------+----------------+
    |           2 |     2.53 sec [3] |   2.52 sec [4] |
    +-------------+------------------+----------------+
    |           3 |     2.02 sec [3] |   2.02 sec [4] |
    +-------------+------------------+----------------+
    |           4 |     1.96 sec [3] |   1.99 sec [4] |
    +-------------+------------------+----------------+

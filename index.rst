...
.. py:class:: CParameter(IntEnum)
    ...
    .. py:attribute:: compressionLevel
        ...
        \* ``ZSTD_CLEVEL_DEFAULT`` is ``3`` in zstd v1.4.8
    ...
    .. py:attribute:: contentSizeFlag
        ...
        Content size must be known at the beginning of compression, such as using:
    ...
    .. py:attribute:: checksumFlag
        ...
        A 4-byte checksum of content is written at end of frame. If decompression verification fails, an error will be raised.
    ...
    .. py:attribute:: overlapLog
        ...
        - 1 means "no overlap"
        - 9 means "full overlap", using a full window size (specified by :py:attr:`CParameter.windowLog`).
...
.. note:: Compression level
    ...
    * From ``1`` to ``22`` (currently), regular compression levels. (Levels >= 20, labeled *ultra*, should be used with caution, as they require more memory.)
    * ``0`` means use default compression level, which is currently ``3`` defined by underlying zstd library.
    * Negative compression levels, which extend the range of speed vs ratio preferences. (The lower the level, the faster the speed, but at the cost of compression.)
    ...
    Compression level does not affect other manually set :ref:`compression parameters<CParameter>`.

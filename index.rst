...
.. py:class:: ZstdCompressor
    ...
    .. py:method:: flush(self, mode=ZstdCompressor.FLUSH_FRAME)
        ...
        :return: A chunk of flushed data if possible, or ``b''`` otherwise.
        :rtype: bytes
...
.. note:: Rich memory mode
    ...
    * Compress data in a single :py:attr:`ZstdCompressor.FLUSH_FRAME` mode.

    Effects:

    * Allocate more memory for output buffer.
    * In single thread compression, 10% ~ 15% faster.
    * In multi-threading compression, maybe the size of output data can be reduced a bit.
    ...
    After enabling this mode, the size of output buffer is provided by ZSTD_compressBound() function, which is larger than input data a little (maximum compressed size in worst case single-pass scenario). For a 100 MB input data, the output buffer size is (100 MB + 409600 bytes). The underlying zstd library has a speed optimization for this output buffer size.

    .. sourcecode:: python

        # used with compress() function
        option = {CParameter.nbWorkers : 8}
        compressed_dat = compress(raw_dat, option, rich_mem=True)
